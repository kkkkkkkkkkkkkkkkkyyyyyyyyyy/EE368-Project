...
class RobotManager:
    ...
    def wait_for_ready(self):
        ...
        while not rospy.is_shutdown():
            try:
                ...
                rospy.loginfo(f"TF 已连接: {self.BASE_FRAME} <- {self.CAMERA_FRAME}")
                break
            except Exception as e:
                rospy.logwarn_throttle(1.0, f"等待 TF {self.BASE_FRAME} <- {self.CAMERA_FRAME}: {e}")
                rospy.sleep(0.2)
    ...
    def pixel_to_base_on_plane(self, u, v):
        ...
        方法：像素反投影成相机射线，然后和 z=Z_INK_PLANE 平面求交。
        """
        ...
        try:
            ...
            rospy.logerr(f"获取 TF 失败: {self.BASE_FRAME} <- {self.CAMERA_FRAME}, {e}")
            return None

        t = trans.transform.translation
        q = trans.transform.rotation

        T_base_cam = tft.quaternion_matrix([q.x, q.y, q.z, q.w])
        T_base_cam[0, 3] = t.x
        T_base_cam[1, 3] = t.y
        T_base_cam[2, 3] = t.z
        ...
        if scale < 0:
            rospy.logerr("交点在相机后方，通常是 TF 方向、camera_frame 或 z_ink_plane 有问题。")
